    ...
    <div className="min-h-screen bg-gray-50 pt-28 pb-20">
      ...
      <div className="bg-primary/5 py-16 border-b border-primary/10">
        <div className="section-container max-w-3xl mx-auto text-center">
          <h1 className="text-4xl md:text-5xl font-bold text-[#8c52ff] mb-6">
            How can we <span className="text-primary">help?</span>
          </h1>
          
          {/* Search Bar */}
          <div className="bg-white p-2 rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 flex items-center mb-6 focus-within:border-primary/30 focus-within:ring-4 focus-within:ring-primary/5 transition-all">
            ...
            <input 
              ...
              className="w-full bg-transparent border-none focus:ring-0 text-gray-700 py-3 px-2 outline-none"
              value={searchQuery}
              onChange={(e) => {
            ...
            />
            <Button 
              href="#" // Prevent default link behavior in real app if used as standard button
              variant="primary"
              size="sm"
              className="bg-primary text-black px-6 py-3 rounded-xl font-semibold shadow-md transition-all duration-300 hidden sm:block"
            >
              Search
            </Button>
          </div>
      ...
      {/* Main Content */}
      <div className="section-container mt-12 max-w-4xl mx-auto">
        ...
        {searchQuery.trim() !== '' && (
          <div className="mb-8 text-center">
            ...
            <p className="text-gray-500 mt-1">Found {filteredFaqs.length} result(s)</p>
          </div>
        )}
          ...
          {filteredFaqs.length > 0 ? (
            filteredFaqs.map((faq) => (
              ...
              >
                ...
                >
                  <span className={`font-bold text-lg transition-colors duration-300 ${openFaq === faq.id ? 'text-primary' : 'text-gray-900 group-hover:text-primary'}`}>
                    {faq.question}
                  </span>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 flex-shrink-0 ml-4 ${openFaq === faq.id ? 'bg-primary/10 text-primary rotate-180' : 'bg-gray-50 text-gray-400 group-hover:bg-primary/5 group-hover:text-primary'}`}>
            ...
            <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
              ...
              <p className="text-gray-500">We couldn't find any FAQs matching your search criteria. Please try another keyword.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="section-container mt-20 max-w-4xl mx-auto">
        <div className="bg-white rounded-3xl p-8 md:p-12 border border-gray-100 shadow-sm text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6">
            <a 
              ...
              className="flex items-center justify-center gap-3 bg-gray-50 hover:bg-primary/5 border border-gray-200 hover:border-primary/30 text-gray-700 hover:text-primary px-8 py-4 rounded-xl font-bold transition-all duration-300 group"
            >
              <HiMail className="text-2xl text-gray-400 group-hover:text-primary transition-colors" />
              Email Support
            </a>
            <button 
              className="flex items-center justify-center gap-3 bg-primary hover:bg-[#8c52ff] text-white px-8 py-4 rounded-xl font-bold shadow-md hover:shadow-lg transition-all duration-300"
            >
