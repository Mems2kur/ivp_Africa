...
export default function JobDetailPage() {
  ...
  function handleToggleSave() {
    if (!session?.email || !job) return;
    const nowSaved = savedJobsApi.toggle(session.email, {
      id: job.id,
      title: job.title,
      company: job.company,
  ...
  function handleMessageEmployer() {
    if (!session?.email || !job) return;
    const conversation = messagesApi.getOrCreateForJob(session.email, {
      id: job.id,
      company: job.company,
      title: job.title,
      initial: job.initial,
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-2xl border border-gray-100 bg-white p-6">
            {applied ? (
              ...
              <button
                ...
                className="w-full rounded-xl bg-[#8A38F5] py-3 text-sm font-semibold text-white transition-colors hover:bg-[#7226e0]"
              >
                Apply Now
              </button>
            )}
            ...
            {!applied && (
              <button
                ...
                className="mt-3 w-full rounded-xl border border-gray-200 py-3 text-sm font-semibold text-gray-900 transition-colors hover:bg-gray-50"
              >
            ...
            <button
              ...
              className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 py-3 text-sm font-semibold text-gray-900 transition-colors hover:bg-gray-50"
            >
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-6">
            ...
            {job.verifiedEmployer && (
              <p className="mt-2 text-sm font-medium text-[#8A38F5]">✓ Verified employer</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-1 text-sm text-gray-500">
              You&apos;re applying to <span className="font-medium text-gray-900">{job.title}</span> at{" "}
              {job.company}.
            </p>
